...
{
   private double screenSize = 1200;
   ...
   private double widthS = Toolkit.getDefaultToolkit().getScreenSize().getWidth();
   private double heightS = Toolkit.getDefaultToolkit().getScreenSize().getHeight();
   public void start(Stage primaryStage)
   {
       ...
       */
      double ratio = widthS/heightS;
      double square = 2.1;
      mandelbrot(-square, square, square/ratio, -square/ratio); //Usually from -2, 2, 2, -2
      /**
       * FOR MATH CLUB:
   ...
   public void showPoint(MouseEvent event)
   {
       ...
       double x1 = rX1+xChange*(event.getX()/widthS);
       double y1 = rY1-yChange*(event.getY()/heightS);
   ...
   public void createAxis(double rangeX, double rangeX2, double rangeY, double rangeY2)
   {
       ...
       Line yAxis = new Line(widthS/2, 0, widthS/2, heightS);
       Line xAxis = new Line(0, heightS/2, widthS,heightS/2);
       ...
       for (int i = 0; i<10; i++)
       {
           change1 = dc.format(rangeX+i*(difference/10));
           change2 = dc.format(rangeY-i*(difference2/10));
           Text marker = new Text(i*widthS/10, (heightS/2)-10, change1);
           Text marker2 = new Text((widthS/2)+10, i*heightS/10, change2+"i");
   ...
   public void createSet(double rangeX, double rangeX2, double rangeY, double rangeY2)
   {
       ...
       double size = 1;
       for (double x = 0; x<=widthS; x+=size)
       {
           for (double y = 0; y<=heightS; y+=size)
           {
               ComplexNumber p = new ComplexNumber((x*(difference/widthS))+rangeX, rangeY-(y*(difference2/heightS)));
               int result = checkStable(p);
               
               if (result == 0)
               {
                  Rectangle c = new Rectangle(x, y, size, size);
